...
async def add_entry_api(name: str, comes_from: str, images: list[UploadFile] = File(...), custom_data: Optional[dict] = {}):
    ...
    # Insert entry in DB
    cursor.execute("INSERT INTO entries (name, image_paths, comes_from, custom_data) VALUES (?, ?, ?, ?)",
                   (name, json.dumps(image_paths), comes_from, json.dumps(custom_data)))
    conn.commit()
...
async def get_entry_api(entry_id: int):
    ...
    return {"id": entry[0], "name": entry[1], "comes_from": entry[2], "image_paths": json.loads(entry[3]), "custom_data": json.loads(entry[4])}
...
async def list_entries_api(page: int = 1):
    ...
    cursor.execute("SELECT id, name, comes_from, image_paths, custom_data FROM entries LIMIT ? OFFSET ?", (PAGE_SIZE, offset))
    ...
    for entry in entries:
        ...
        data = {"id": entry[0], "name": entry[1], "comes_from": entry[2], "image_paths": imgs, "custom_data": json.loads(entry[4])}
